...
func TestGetCustomPkiDir(t *testing.T) {
	root := "/tmp"
	PkiConfig.BaseDir = root
	baseDir := GetPkiDir()
...
func TestGetPkiDir(t *testing.T) {
	PkiConfig.BaseDir = GetHomeDir()
	homeDir := GetHomeDir()
	baseDir := GetPkiDir()
...
func TestGetCADir(t *testing.T) {
	dir := GetCADir()
	expected := fmt.Sprintf("%s/ca", GetPkiDir())
	if dir != expected {
